...
def view_config():
    ...
    return render_template('admin/view_config.html', config=config, last_round=last_round, last_round_start_time=last_round_start_time, last_tick=last_tick, last_tick_start_time=last_tick_start_time)
...
def update_config():
    ...
    if isStartingChallenge:
        # Start the challenge with first tick and precise scheduler
        from app import start_challenge_with_first_tick
        success = start_challenge_with_first_tick()
        if success:
            print("Challenge started - first tick executed and precise scheduler initiated")
        else:
            flash('Error starting challenge', 'danger')
            return redirect(url_for('view_config'))
    else:
        # Stop the scheduler when challenge is stopped
        from app import stop_tick_scheduler
        stop_tick_scheduler()
        print("Challenge stopped - scheduler stopped")
    ...
    try:
        ...
        for user in users:
            for challenge in challenges:
                ...
                try:
                    check_response = requests.get(check_url, auth=(os.getenv('API_USERNAME', os.getenv('ADMIN_USERNAME')), os.getenv('API_PASSWORD', os.getenv('ADMIN_PASSWORD'))), timeout=10)
                    if check_response.status_code == 200 and check_response.json().get('success'):
                        status = 'up'  # Success status
                    else:
                        status = 'error'  # Error status
                    print(f"Check {challenge.name} for {user.host_ip}: {status}")
                except requests.RequestException as e:
                    print(f"Error checking {challenge.name} for {user.host_ip}: {e}")
                    status = 'down'  # Error status if request fails
                except Exception as e:
                    print(f"Unexpected error checking {challenge.name} for {user.host_ip}: {e}")
                    status = 'down'
                ...
                for user in users:
                    try:
                        ...
                        response = requests.post(flag_distribution_url, 
                                               json={'flag': flag_string, 'challenge': challenge.name}, 
                                               auth=(os.getenv('API_USERNAME', os.getenv('ADMIN_USERNAME')), os.getenv('API_PASSWORD', os.getenv('ADMIN_PASSWORD'))),
                                               timeout=10)
                        ...
                        print(f"Unexpected error distributing flag for {challenge.name} to {user.host_ip}: {e}")

        # Commit all changes to the database
        db.session.commit()

        # Invalidate scoreboard cache when new data is added
        # try:
        #     from app.cache import scoreboard_cache
        ...
        print(f"Error in next_tick: {e}")
        db.session.rollback()
        return f"[{pytz.timezone('Asia/Jakarta').localize(datetime.now())}] Error in tick processing: {e}"
...
@app.route('/manual_tick', methods=['POST'])
@login_required
def manual_tick():
    """Manual trigger for next tick - useful for testing"""
    if not current_user.is_admin:
        flash('You do not have permission to perform this action.', 'danger')
        return redirect(url_for('dashboard'))
    ...
    try:
        result = next_tick()
        flash(f'Manual tick executed: {result}', 'success')
    except Exception as e:
        flash(f'Error executing manual tick: {e}', 'danger')
